...
make changelogs # regenerate per-policy-domain changelogs in docs/changelogs/
...
Per-policy-domain changelogs live under
[docs/changelogs/](docs/changelogs/README.md) - one file per
`policy/<domain>/` directory, filtered by path so consumers adopting only
one domain can read just what's relevant to them. The top-level
[CHANGELOG.md](CHANGELOG.md) remains the canonical release log
